...
export default function NewestNewsSection() {
  ...
  if (isLoading) {
    return (
      <article className="w-full lg:w-8/12  bg-white/80 px-10 py-10">
  ...
  return (
    <article className="w-full lg:w-8/12 border bg-white/80 lg:px-20 p-10 py-10">
      <h2 className="text-4xl font-bold text-slate-700 text-center">Berita Terbaru</h2>
      <Carousel
        plugins={[
          Autoplay({
        ...
        <CarouselContent>
          {
            data && data.data?.map((news: News, index: number) => (
              <CarouselItem key={index} className='basis-1/2 lg:basis-1/2'>
                {/* <div className='bg-white p-5 rounded h-40 lg:h-[500px] shadow-2xl flex flex-col justify-between'>
                ...
                <div className='bg-white rounded h-60 lg:h-[500px] shadow-2xl flex flex-col justify-between relative overflow-hidden'>
                  ...
                  <div className='mt-5 relative flex justify-center z-20 items-center bottom-8'>
                    <Button variant={"outline"} className='text-lg md:px-5 md:py-5 rounded-full'>Read More</Button>
                  </div>
                </div>
              </CarouselItem>
            ))
